...
const [
  ...
  read('supabase/migrations/20260826103857_restrict_security_definer_execution.sql'),
...
const contracts = [
  ...
  ['future public functions deny anonymous execution by default', securityDefinerPermissions, /alter default privileges for role postgres in schema public[\s\S]*revoke execute on functions from public, anon/],
  ['existing security-definer functions deny anonymous execution', securityDefinerPermissions, /where namespace\.nspname = 'public'[\s\S]*routine\.prosecdef[\s\S]*revoke execute on function %s from public, anon/],
  ['the action transition helper has an immutable search path', securityDefinerPermissions, /alter function public\.action_transition_allowed\(text, text\)[\s\S]*set search_path = pg_catalog/],
