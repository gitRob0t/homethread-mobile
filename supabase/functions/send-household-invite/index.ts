...
Deno.serve(async (request) => {
  ...
  if (request.method === 'GET') {
    ...
    if (!/^[a-f0-9]{48}$/i.test(token)) return invitePage('This invitation link is invalid.', null, 400);
    const tokenHash = await sha256(token);
    const { data: invitation } = await admin
      .from('invitations')
      .select('status, expires_at, households(name)')
    ...
    if (!invitation || invitation.status !== 'pending' || new Date(invitation.expires_at) <= new Date()) {
      return invitePage('This invitation has expired or was already used.', null, 410);
    }
    return invitePage(`Join ${household?.name || 'your family'} on Coho`, token);
  }
...
function invitePage(title: string, token: string | null, status = 200) {
  const appUrl = token ? `coho://invite/${token}` : null;
...
a{display:block;margin-top:24px;padding:16px;border-radius:16px;background:#fff;color:#182033;text-decoration:none;font-weight:800}
</style></head><body><main><div class="mark">✦</div><h1>${safeTitle}</h1>
<p class="muted">${appUrl ? 'Tap below on the iPhone or Android device where Coho is installed. Sign in with the email that received the invitation.' : 'Ask the household owner to send a new secure invitation.'}</p>
${appUrl ? `<a href="${appUrl}">Open invitation in Coho</a>` : ''}
</main>${appUrl ? `<script>setTimeout(()=>location.href=${JSON.stringify(appUrl)},350)</script>` : ''}</body></html>`, {
    status,
    headers: {
