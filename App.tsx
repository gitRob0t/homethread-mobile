    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.screen}>
          ...
          {tab === 'Chores' && <ChoresScreen styles={styles} chores={chores} rewardMember={rewardMember} setRewardMember={setRewardMember} selectedRewards={selectedRewards} onSelectReward={(member: string, reward: string) => { const next = { ...selectedRewards, [member]: reward }; setSelectedRewards(next); AsyncStorage.setItem('coho-reward-goals', JSON.stringify(next)); showNotice(`${member} picked a new reward goal`); }} onToggle={(id: string) => setChores((items) => items.map((item) => item.id === id ? { ...item, done: !item.done } : item))} />}
...
function ChoresScreen({ styles, chores, onToggle, rewardMember, setRewardMember, selectedRewards, onSelectReward }: any) {
