...
type Tab = 'Today' | 'Calendar' | 'Chores' | 'Ace' | 'More';
...
function OutrSpaceApp() {
  ...
  async function saveQuickAdd() {
    ...
    try {
      ...
      if (quickAddType === 'Chore') await createChore(householdId, userId, quickAddTitle, quickAddDetails, quickAddAssignee || null);
      ...
      setQuickAddTitle(''); setQuickAddDetails(''); setQuickAddWhen(''); setQuickAddAssignee('');
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.app}>
        ...
        <View style={styles.screen}>
          ...
          {!dataError && !dataLoading && tab === 'Today' && <TodayScreen theme={theme} styles={styles} events={events} chores={chores} members={members} onAskAce={() => setTab('Ace')} onProfile={setActiveProfile} />}
          ...
          {!dataError && !dataLoading && tab === 'Ace' && <AceScreen styles={styles} events={events} chores={chores} messages={messages} onFamilyText={() => { setQuickAddType('Message'); setQuickAddOpen(true); }} />}
      ...
      <QuickAddModal
        ...
        setAssignee={setQuickAddAssignee}
        onSave={saveQuickAdd}
        busy={quickAddBusy}
        dark={dark}
      />
      <ProfileModal member={activeProfile} visible={!!activeProfile} onClose={() => setActiveProfile(null)} />
      <SearchModal visible={searchOpen} query={searchQuery} setQuery={setSearchQuery} onClose={() => setSearchOpen(false)} events={events} chores={chores} notes={notes} messages={messages} />
    </SafeAreaView>
  );
}

function Header({ title, styles, dark, onTheme, onSearch, onAdd, onBack }: any) {
  return <View style={styles.header}>
    ...
    <View style={styles.headerButtons}>
      <Pressable accessibilityLabel="Search OutrSPACE" onPress={onSearch} style={styles.iconButton}><Ionicons name="search-outline" size={20} color={styles.iconColor.color} /></Pressable>
...
function TodayScreen({ theme, styles, events, chores, members, onAskAce, onProfile }: any) {
  ...
  return <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.sectionHead}><Text style={styles.sectionTitle}>Where everyone is</Text><Text style={styles.link}>Open Family Places</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.peopleScroll}>{members.map((member: Member, index: number) => <Pressable key={member.user_id} onPress={() => onProfile(member)} style={styles.personCard}><Avatar member={member} size={56} color={['#2257F4', '#D7550D', '#168866', '#6E3AE2'][index % 4]} /><Text numberOfLines={1} style={styles.familyName}>{member.profiles[0]?.display_name || 'Family'}</Text><Text style={styles.personView}>VIEW</Text></Pressable>)}</ScrollView>
...
function Avatar({ member, size = 40, color = '#2257F4' }: { member: Member; size?: number; color?: string }) {
  const profile = member.profiles[0];
  return profile?.avatar_url ? <Image source={{ uri: profile.avatar_url }} style={{ width: size, height: size, borderRadius: size / 2 }} /> : <View style={{ width: size, height: size, borderRadius: size / 2, alignItems: 'center', justifyContent: 'center', backgroundColor: `${color}18`, borderWidth: 1, borderColor: `${color}40` }}><Text style={{ color, fontSize: size * .32, fontWeight: '900' }}>{initials(profile?.display_name || 'Family')}</Text></View>;
}

function ProfileModal({ member, visible, onClose }: { member: Member | null; visible: boolean; onClose: () => void }) {
  if (!member) return null;
  const name = member.profiles[0]?.display_name || 'Family member';
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><View style={profileStyles.backdrop}><Pressable style={profileStyles.dismiss} onPress={onClose} /><View style={profileStyles.sheet}><View style={profileStyles.handle} /><Avatar member={member} size={72} /><Text style={profileStyles.name}>{name}</Text><Text style={profileStyles.role}>{member.role === 'owner' ? 'Household owner' : member.role === 'admin' ? 'Adult admin' : 'Family member'}</Text><View style={profileStyles.actions}><Pressable style={profileStyles.action}><Ionicons name="chatbubble-outline" size={20} color="#2257F4" /><Text style={profileStyles.actionText}>Message</Text></Pressable><Pressable style={profileStyles.action}><Ionicons name="calendar-outline" size={20} color="#2257F4" /><Text style={profileStyles.actionText}>Schedule</Text></Pressable><Pressable style={profileStyles.action}><Ionicons name="create-outline" size={20} color="#2257F4" /><Text style={profileStyles.actionText}>Edit profile</Text></Pressable></View><Pressable onPress={onClose} style={profileStyles.close}><Text style={profileStyles.closeText}>Done</Text></Pressable></View></View></Modal>;
}

function SearchModal({ visible, query, setQuery, onClose, events, chores, notes, messages }: any) {
  const all = [...events.map((x: FamilyEvent) => ({ type: 'Event', title: x.title })), ...chores.map((x: FamilyChore) => ({ type: 'Mission', title: x.title })), ...notes.map((x: FamilyNote) => ({ type: 'Note', title: x.title })), ...messages.map((x: FamilyMessage) => ({ type: 'Message', title: x.body }))];
  const matches = query.trim() ? all.filter((x) => x.title.toLowerCase().includes(query.trim().toLowerCase())) : all.slice(0, 8);
  return <Modal visible={visible} animationType="slide" onRequestClose={onClose}><SafeAreaView style={profileStyles.searchPage}><View style={profileStyles.searchHead}><TextInput autoFocus value={query} onChangeText={setQuery} placeholder="Search your family space" placeholderTextColor="#8B93A5" style={profileStyles.searchInput} /><Pressable onPress={onClose}><Text style={profileStyles.cancel}>Done</Text></Pressable></View><Text style={profileStyles.searchLabel}>{query ? 'Results' : 'Recent in OutrSPACE'}</Text><ScrollView>{matches.map((item) => <View key={`${item.type}-${item.title}`} style={profileStyles.result}><Text style={profileStyles.resultType}>{item.type.toUpperCase()}</Text><Text style={profileStyles.resultTitle}>{item.title}</Text></View>)}</ScrollView></SafeAreaView></Modal>;
}

function AceScreen({ styles, events, chores, messages, onFamilyText }: any) {
  const [prompt, setPrompt] = useState('');
  const [answer, setAnswer] = useState('');
  const ask = (value: string) => { const next = value || prompt; setPrompt(next); setAnswer(`I found ${events.length} calendar item${events.length === 1 ? '' : 's'}, ${chores.filter((x: FamilyChore) => x.status === 'open').length} open mission${chores.filter((x: FamilyChore) => x.status === 'open').length === 1 ? '' : 's'}, and ${messages.length} family update${messages.length === 1 ? '' : 's'}. I’ll always show the proposed action before changing anything.`); };
  return <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}><ScrollView contentContainerStyle={styles.scrollContent}><Text style={styles.aceTitle}>Ace</Text><View style={styles.aceOrb}><Ionicons name="sparkles" size={42} color="#fff" /></View><Text style={styles.aceEyebrow}>ACE IS LISTENING</Text><Text style={styles.aceHeadline}>Tell Ace the messy version.</Text><Text style={styles.aceDescription}>Ask in plain language. Ace reads live household context and proposes the next action for approval.</Text><View style={styles.acePromptCard}><Text style={styles.acePromptTitle}>Try asking Ace</Text><View style={styles.chips}>{['What am I missing tomorrow?', 'Plan our week', 'Turn a message into a plan', 'What needs approval?'].map((item) => <Pressable key={item} onPress={() => ask(item)} style={styles.chip}><Text style={styles.chipText}>{item}</Text></Pressable>)}</View></View>{answer ? <View style={styles.aceAnswer}><Text style={styles.acePromptTitle}>Ace’s read</Text><Text style={styles.muted}>{answer}</Text><Pressable onPress={onFamilyText} style={styles.aceAction}><Text style={styles.saveButtonText}>Share a family update</Text></Pressable></View> : null}</ScrollView><View style={styles.aceCompose}><TextInput value={prompt} onChangeText={setPrompt} placeholder="Tell Ace what’s going on…" placeholderTextColor="#8B93A5" style={styles.aceInput} multiline /><Pressable onPress={() => ask('')} style={styles.aceSend}><Ionicons name="arrow-up" size={22} color="#fff" /></Pressable></View></KeyboardAvoidingView>;
}

function CalendarScreen({ styles, events, onDelete }: any) {
  const [view, setView] = useState<'Day' | 'Week' | 'Month' | 'Year'>('Week');
  const daysShown = view === 'Day' ? 1 : view === 'Week' ? 7 : view === 'Month' ? 30 : 365;
  const start = new Date();
  const visibleEvents = events.filter((event: FamilyEvent) => new Date(event.starts_at).getTime() <= start.getTime() + daysShown * 86400000);
  return <ScrollView contentContainerStyle={styles.scrollContent}><View style={styles.calendarHero}><Text style={styles.calendarHeroLabel}>{view.toUpperCase()} VIEW</Text><Text style={styles.calendarHeroTitle}>{visibleEvents.length} things in view.</Text><Text style={styles.calendarHeroText}>Your connected sources will appear here only after they are authorized.</Text></View><View style={styles.viewToggle}>{(['Day', 'Week', 'Month', 'Year'] as const).map((item) => <Pressable key={item} onPress={() => setView(item)} style={[styles.viewOption, view === item && styles.viewOptionActive]}><Text style={[styles.viewOptionText, view === item && styles.viewOptionTextActive]}>{item}</Text></Pressable>)}</View><Text style={styles.sectionTitle}>{view === 'Day' ? 'Today' : `Next ${daysShown} days`}</Text>{visibleEvents.length ? visibleEvents.map((event: FamilyEvent, index: number) => <View key={event.id} style={styles.timelineRow}><View style={[styles.timelineLine, { backgroundColor: ['#2257F4', '#19A47B', '#FF7A2E', '#6E3AE2'][index % 4] }]} /><Text style={styles.timelineTime}>{formatTime(event.starts_at)}</Text><View style={styles.flex}><Text style={styles.timelineTitle}>{event.title}</Text><Text style={styles.muted}>{new Date(event.starts_at).toLocaleDateString()} · {event.location || event.details || 'Family event'}</Text></View><Pressable onPress={() => onDelete(event.id)}><Ionicons name="trash-outline" size={18} color={styles.iconColor.color} /></Pressable></View>) : <View style={styles.emptyCard}><Ionicons name="calendar-outline" size={24} color="#2257F4" /><Text style={styles.emptyTitle}>Nothing scheduled in this view</Text><Text style={styles.muted}>Add a family event using the + button.</Text></View>}</ScrollView>;
...
function BottomTabs({ tab, setTab, styles }: any) {
  const tabs = [['Today', 'home'], ['Calendar', 'calendar'], ['Ace', 'sparkles'], ['Chores', 'checkbox'], ['More', 'ellipsis-horizontal-circle']];
...
const profileStyles = StyleSheet.create({
  backdrop: { flex: 1, backgroundColor: '#0C111D77', justifyContent: 'flex-end' }, dismiss: { flex: 1 }, sheet: { backgroundColor: '#FFFDF8', borderTopLeftRadius: 30, borderTopRightRadius: 30, padding: 24, alignItems: 'center' }, handle: { width: 42, height: 4, borderRadius: 2, backgroundColor: '#DDE1EA', marginBottom: 20 }, name: { color: '#14213D', fontSize: 25, fontWeight: '900', marginTop: 12 }, role: { color: '#6D7486', fontSize: 13, marginTop: 5 }, actions: { width: '100%', flexDirection: 'row', gap: 9, marginTop: 24 }, action: { flex: 1, minHeight: 82, borderRadius: 16, backgroundColor: '#F4F7FD', alignItems: 'center', justifyContent: 'center', gap: 7 }, actionText: { color: '#2257F4', fontSize: 11, fontWeight: '800' }, close: { width: '100%', minHeight: 50, borderRadius: 15, backgroundColor: '#2257F4', alignItems: 'center', justifyContent: 'center', marginTop: 18 }, closeText: { color: '#fff', fontSize: 13, fontWeight: '900' }, searchPage: { flex: 1, backgroundColor: '#FFF8E9', padding: 18 }, searchHead: { flexDirection: 'row', alignItems: 'center', gap: 12 }, searchInput: { flex: 1, minHeight: 50, borderRadius: 15, borderWidth: 1, borderColor: '#DDE1EA', backgroundColor: '#fff', color: '#14213D', paddingHorizontal: 14 }, cancel: { color: '#2257F4', fontSize: 13, fontWeight: '800' }, searchLabel: { color: '#14213D', fontSize: 18, fontWeight: '900', marginTop: 26, marginBottom: 12 }, result: { backgroundColor: '#fff', borderRadius: 15, padding: 15, marginBottom: 9, borderWidth: 1, borderColor: '#EADFC9' }, resultType: { color: '#2257F4', fontSize: 9, fontWeight: '900', letterSpacing: 1 }, resultTitle: { color: '#14213D', fontSize: 14, fontWeight: '800', marginTop: 5 },
});
...
function createStyles(t: Theme) {
  return StyleSheet.create({
    ...
    modalBackdrop: { flex: 1, backgroundColor: '#0C111D88', justifyContent: 'flex-end' }, modalDismiss: { flex: 1 }, modalSheet: { backgroundColor: t.surfaceStrong, borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingHorizontal: 19, paddingTop: 9, paddingBottom: Platform.OS === 'ios' ? 28 : 18 }, modalHandle: { width: 39, height: 4, borderRadius: 2, backgroundColor: t.line, alignSelf: 'center', marginBottom: 15 }, modalHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }, modalTitle: { color: t.text, fontSize: 23, fontWeight: '800', letterSpacing: -.7 }, typeTabs: { flexDirection: 'row', borderRadius: 14, padding: 4, backgroundColor: t.canvas, marginTop: 19 }, typeTab: { flex: 1, minHeight: 38, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, typeTabActive: { backgroundColor: t.surfaceStrong }, typeTabText: { color: t.muted, fontSize: 10, fontWeight: '700' }, typeTabTextActive: { color: t.primary }, fieldLabel: { color: t.muted, fontSize: 9, fontWeight: '800', marginTop: 15, marginBottom: 6 }, modalInput: { minHeight: 46, borderRadius: 13, borderWidth: 1, borderColor: t.line, backgroundColor: t.surface, color: t.text, paddingHorizontal: 12 }, modalTextArea: { minHeight: 83, paddingTop: 12, textAlignVertical: 'top' }, assignRow: { gap: 8, paddingVertical: 2 }, assignChip: { borderRadius: 18, paddingHorizontal: 12, paddingVertical: 9, borderWidth: 1, borderColor: t.line }, assignChipActive: { borderColor: t.primary, backgroundColor: `${t.primary}14` }, assignChipText: { color: t.text, fontSize: 11, fontWeight: '800' }, saveButton: { minHeight: 48, borderRadius: 15, backgroundColor: t.primary, alignItems: 'center', justifyContent: 'center', marginTop: 18 }, saveButtonText: { color: '#fff', fontSize: 12, fontWeight: '800' }, peopleScroll: { gap: 10, paddingRight: 18 }, personCard: { width: 112, minHeight: 132, borderWidth: 1, borderColor: t.line, backgroundColor: t.surface, borderRadius: 20, padding: 13, justifyContent: 'space-between' }, personView: { color: t.primary, fontSize: 9, fontWeight: '900' }, aceTitle: { color: t.text, fontSize: 44, fontWeight: '900', marginTop: 14 }, aceOrb: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#3C82E8', alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 32, shadowColor: '#3C82E8', shadowOpacity: .35, shadowRadius: 24 }, aceEyebrow: { color: t.primary, fontSize: 11, fontWeight: '900', letterSpacing: 2, textAlign: 'center', marginTop: 20 }, aceHeadline: { color: t.text, fontSize: 30, fontWeight: '900', textAlign: 'center', marginTop: 18 }, aceDescription: { color: t.muted, fontSize: 15, lineHeight: 22, textAlign: 'center', marginTop: 12 }, acePromptCard: { borderRadius: 24, backgroundColor: t.surface, borderWidth: 1, borderColor: t.line, padding: 18, marginTop: 22 }, acePromptTitle: { color: t.text, fontSize: 18, fontWeight: '900' }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 9, marginTop: 13 }, chip: { borderRadius: 24, backgroundColor: '#E9F1FF', paddingHorizontal: 14, paddingVertical: 11 }, chipText: { color: '#2257C4', fontSize: 13, fontWeight: '800' }, aceAnswer: { borderRadius: 22, padding: 18, backgroundColor: '#EAF9FB', gap: 10 }, aceAction: { alignSelf: 'flex-start', borderRadius: 12, backgroundColor: t.primary, paddingHorizontal: 14, paddingVertical: 11, marginTop: 4 }, aceCompose: { minHeight: 76, paddingHorizontal: 16, paddingVertical: 10, flexDirection: 'row', gap: 10, borderTopWidth: 1, borderColor: t.line, backgroundColor: t.surfaceStrong }, aceInput: { flex: 1, maxHeight: 74, borderWidth: 1, borderColor: t.line, backgroundColor: t.surface, borderRadius: 18, color: t.text, padding: 12 }, aceSend: { width: 52, height: 52, borderRadius: 26, backgroundColor: t.primary, alignItems: 'center', justifyContent: 'center', alignSelf: 'flex-end' }, calendarHero: { borderRadius: 26, backgroundColor: '#102F4E', padding: 22, minHeight: 150 }, calendarHeroLabel: { color: '#A9C7EA', fontSize: 10, fontWeight: '900', letterSpacing: 1.4 }, calendarHeroTitle: { color: '#fff', fontSize: 30, fontWeight: '900', marginTop: 14 }, calendarHeroText: { color: '#C4D7EA', fontSize: 11, lineHeight: 16, marginTop: 8 }, viewToggle: { flexDirection: 'row', padding: 4, backgroundColor: t.surface, borderRadius: 16, borderWidth: 1, borderColor: t.line }, viewOption: { flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: 12 }, viewOptionActive: { backgroundColor: t.primary }, viewOptionText: { color: t.muted, fontSize: 11, fontWeight: '800' }, viewOptionTextActive: { color: '#fff' },
  });
}
